...
(async()=>{
  ...
  try{
    ...
    assert.equal(await page.locator('.part-card').count(),16);
    ...
    await page.waitForSelector('.part-card');
    assert.equal(await page.locator('.part-card').count(),19);
    await page.locator('[data-category="fluids"]').click();
    assert.ok(await page.locator('.part-card').count()>=5);
    await page.locator('[data-category="all"]').click();
    await page.locator('#parts-search').fill('Opteon YF');
    await page.waitForFunction(()=>document.querySelectorAll('.part-card').length===1);
    assert.ok((await page.locator('.part-card').first().innerText()).includes('以机舱标签'));
